...
class ServerManager extends PsychObject {
  ...
  openSession() {
    ...
    return new Promise((resolve, reject) => {
      const url =
        ...
        "/api/v2/experiments/" +
        encodeURIComponent(self._psychoJS.config.experiment.fullpath) +
        "/sessions";
  ...
  closeSession(isCompleted = false, sync = false) {
    ...
    const url =
      this._psychoJS.config.pavlovia.URL +
      "/api/v2/experiments/" +
      encodeURIComponent(this._psychoJS.config.experiment.fullpath) +
      "/sessions/" +
  ...
  getResource(name) {
    ...
    if (typeof path_data === "undefined") throw Object.assign(response, { error: "unknown resource" });
  ...
  setStatus(status) {
    ...
    if (!statusKey) throw Object.assign(response, { error: "status must be a symbol" });
    if (!ServerManager.Status.hasOwnProperty(statusKey)) throw Object.assign(response, { error: "unknown status" });
    this._status = status;
      ...
      try {
        if (self._psychoJS.config.environment === ExperimentHandler.Environment.SERVER) {
          if (resources.length === 0) {
            ...
            for (const name of serverResponse.resources) self._resources.set(name, { path: serverResponse.resourceDirectory + "/" + name });
          } else {
            for (const { name, path } of resources) self._resources.set(name, { path });
          }
        } else {
          for (const { name, path } of resources) self._resources.set(name, { path });
        }
        self._nbResources = self._resources.size;
        for (const name of self._resources.keys()) this._psychoJS.logger.debug("resource:", name, self._resources.get(name).path);
  ...
  uploadData(key, value) {
    ...
    return new Promise((resolve, reject) => {
      const url =
        self._psychoJS.config.pavlovia.URL +
        "/api/v2/experiments/" +
        encodeURIComponent(self._psychoJS.config.experiment.fullpath) +
        "/sessions/" +
        self._psychoJS.config.session.token +
        "/results";
      $.post(url, data, null, "json")
  ...
  _downloadRegisteredResources() {
    ...
    for (const [name, path_data] of this._resources) {
      ...
      } else manifest.push({ id: name, src: path_data.path });
    }
    if (manifest.length > 0) this._resourceQueue.loadManifest(manifest);
    else {
...
class Logger {
  ...
  log(msg, level, time, obj) {
    if (typeof time === "undefined") time = MonotonicClock.getReferenceTime();
...
class Window extends PsychObject {
  ...
  constructor({ psychoJS, name, fullscr = false, color = new Color("black"), units = "pix", waitBlanking = false, autoLog = true } = {}) {
    super(psychoJS, name);
    this._msgToBeLogged = [];
    this._drawList = [];
    ...
    document.addEventListener("fullscreenchange", (event) => {
      ...
      for (const stimulus of self._drawList) stimulus._needUpdate = true;
    });
    if (this._autoLog) this._psychoJS.experimentLogger.exp(`Created ${this.name} = ${this.toString()}`);
  }
  close() {
  ...
  adjustScreenSize() {
    if (this.fullscr) {
      ...
      } else if (typeof document.documentElement.mozRequestFullScreen === "function") document.documentElement.mozRequestFullScreen();
      else if (typeof document.documentElement.webkitRequestFullscreen === "function") document.documentElement.webkitRequestFullscreen();
      else if (typeof document.documentElement.msRequestFullscreen === "function") document.documentElement.msRequestFullscreen();
      else this.psychoJS.logger.warn("Unable to go fullscreen.");
    }
  }
  closeFullScreen() {
    if (this.fullscr) {
      ...
      else this.psychoJS.logger.warn("Unable to close fullscreen.");
    }
  }
  ...
  render() {
    ...
    for (let callback of this._flipCallbacks) callback["function"](...callback["arguments"]);
    this._flipCallbacks = [];
  ...
  _fullRefresh() {
    ...
    for (const stimulus of this._drawList) stimulus.refresh();
    this._refresh();
  }
  _setupPixi() {
...
class GUI {
  ...
  DlgFromDict({ logoUrl, text, dictionary, title }) {
    ...
    return () => {
      ...
      if (t >= 0.0 && self._dialogComponent.status === PsychoJS.Status.NOT_STARTED) {
        ...
        for (const key in dictionary) {
          ...
          if (!inUrl) {
            ...
            if (Array.isArray(value)) {
              ...
              for (const option of value) htmlCode += "<option>" + option + "</option>";
              htmlCode += "</select>";
        ...
        for (const key of this._requiredKeys) {
          ...
          if (input) input.onchange = (event) => GUI._onKeyChange(self, event);
        }
            ...
            buttons: [
              {
                ...
                click: function () {
                  for (const key in dictionary) {
                    ...
                    if (input) dictionary[key] = input.value;
                  }
      ...
      if (self._dialogComponent.status === PsychoJS.Status.FINISHED) return Scheduler.Event.NEXT;
      else return Scheduler.Event.FLIP_REPEAT;
    };
  }
  dialog({ message, warning, error, showOK = true, onOK } = {}) {
    this.destroyDialog();
    let htmlCode;
    let titleColour;
  ...
  _updateOkButtonStatus() {
    if (
      ...
      (this._allResourcesDownloaded && this._nbSetRequiredKeys >= this._requiredKeys.length)
    ) {
  ...
  static _onKeyChange(gui, event) {
    ...
    if (typeof value !== "undefined" && value.length > 0) gui._nbSetRequiredKeys++;
    else gui._nbSetRequiredKeys--;
...
class PsychoJS {
  ...
  constructor({ debug = true, collectIP = false, topLevelStatus = true } = {}) {
  ...
  async start({ configURL = "config.json", expName = "UNKNOWN", expInfo, resources = [] } = {}) {
    ...
    try {
      ...
      if (this._collectIP) this._getParticipantIPInfo();
      else {
        this._IP = {
          IP: "X",
          hostname: "X",
    ...
    try {
      ...
      this._gui.dialog({
        ...
        onOK: () => {
          ...
          if (isCompleted && typeof self._completionUrl !== "undefined") window.location = self._completionUrl;
          else if (!isCompleted && typeof self._cancellationUrl !== "undefined") window.location = self._cancellationUrl;
        },
      });
    } catch (error) {
      console.error(error);
  ...
  async _configure(configURL, name) {
    ...
    try {
      ...
      if (experimentUrl.indexOf("https://run.pavlovia.org/") === 0 || experimentUrl.indexOf("https://pavlovia.org/run/") === 0) {
        ...
        if (!("experiment" in this._config)) throw "missing experiment block in configuration";
        if (!("name" in this._config.experiment)) throw "missing name in experiment block in configuration";
        if (!("fullpath" in this._config.experiment)) throw "missing fullpath in experiment block in configuration";
        if (!("pavlovia" in this._config)) throw "missing pavlovia block in configuration";
        if (!("URL" in this._config.pavlovia)) throw "missing URL in pavlovia block in configuration";
...
class EventManager {
  constructor(psychoJS) {
    this._psychoJS = psychoJS;
    for (const keyName in EventManager._pygletMap) EventManager._reversePygletMap[EventManager._pygletMap[keyName]] = keyName;
    this._keyBuffer = [];
  ...
  getKeys({ keyList = null, timeStamped = false } = {}) {
    ...
    for (let i = 0; i < this._keyBuffer.length; ++i) {
      ...
      if (keyId != null) {
        if (timeStamped) keys.push([keyId, key.timestamp]);
        else keys.push(keyId);
    ...
    renderer.view.addEventListener(
      ...
      (event) => {
        ...
        self._mouseInfo.buttons.times[event.button] =
          self._psychoJS._monotonicClock.getTime() - self._mouseInfo.buttons.clocks[event.button].getLastResetTime();
        self._mouseInfo.pos = [event.offsetX, event.offsetY];
        this._psychoJS.experimentLogger.data(
          "Mouse: " + event.button + " button down, pos=(" + self._mouseInfo.pos[0] + "," + self._mouseInfo.pos[1] + ")"
        );
      },
      false
    );
  ...
  static pyglet2w3c(pygletKeyList) {
    ...
    for (let i = 0; i < pygletKeyList.length; i++) {
      if (typeof EventManager._pygletMap[pygletKeyList[i]] === "undefined") w3cKeyList.push(pygletKeyList[i]);
      else w3cKeyList.push(EventManager._pygletMap[pygletKeyList[i]]);
    }
    return w3cKeyList;
  }
  static w3c2pyglet(code) {
    if (code in EventManager._reversePygletMap) return EventManager._reversePygletMap[code];
    else return "N/A";
  }
...
class Keyboard extends PsychObject {
  constructor({ psychoJS, bufferSize = 10000, waitForStart = false, clock, autoLog = false } = {}) {
    super(psychoJS);
    if (typeof clock === "undefined") clock = new Clock();
...
class MinimalStim extends PsychObject {
  constructor({ name, win, autoDraw = false, autoLog = win.autoLog } = {}) {
...
class Mouse extends PsychObject {
  ...
  getRel() {
    if (typeof this._lastPos === "undefined") return this.getPos();
    else {
  ...
  getPressed(getTime = false) {
    ...
    if (!getTime) return buttonPressed;
    else {
  ...
  mouseMoved(distance, reset = false) {
    ...
    if (typeof reset === "boolean" && reset == false) {
      if (typeof distance === "undefined") return this._prevPos[0] != this._lastPos[0] || this._prevPos[1] != this._lastPos[1];
      else {
        if (typeof distance === "number") {
          this._movedistance = Math.sqrt(
            (this._prevPos[0] - this._lastPos[0]) * (this._prevPos[0] - this._lastPos[0]) +
              (this._prevPos[1] - this._lastPos[1]) * (this._prevPos[1] - this._lastPos[1])
          );
