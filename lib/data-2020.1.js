...
class ExperimentHandler extends PsychObject {
  ...
  nextEntry(snapshots) {
    if (typeof snapshots !== "undefined") {
      ...
      for (const snapshot of snapshots) {
        ...
        for (let a in attributes) if (attributes.hasOwnProperty(a)) this._currentTrialData[a] = attributes[a];
      }
    } else {
      for (const loop of this._unfinishedLoops) {
        const attributes = ExperimentHandler._getLoopAttributes(loop);
        for (const a in attributes) if (attributes.hasOwnProperty(a)) this._currentTrialData[a] = attributes[a];
      }
    }
    for (let a in this.extraInfo) if (this.extraInfo.hasOwnProperty(a)) this._currentTrialData[a] = this.extraInfo[a];
  ...
  async save({ attributes = [] } = {}) {
    ...
    if (attributes.length === 0) {
      ...
      for (let l = 0; l < this._loops.length; l++) {
        ...
        for (let a in loopAttributes) if (loopAttributes.hasOwnProperty(a)) attributes.push(a);
      }
      for (let a in this.extraInfo) {
        if (this.extraInfo.hasOwnProperty(a)) attributes.push(a);
      }
    }
    ...
    const __projectId =
      typeof gitlabConfig !== "undefined" && typeof gitlabConfig.projectId !== "undefined" ? gitlabConfig.projectId : undefined;
    if (this._psychoJS.config.experiment.saveFormat === ExperimentHandler.SaveFormat.CSV) {
      ...
      else saveData(key, csv, "text/csv"); // Added by BCS June 2020
      ...
      for (let r = 0; r < this._trialsData.length; r++) {
        ...
        for (let h = 0; h < attributes.length; h++) doc[attributes[h]] = this._trialsData[r][attributes[h]];
        documents.push(doc);
      }
      if (this._psychoJS.getEnvironment() === ExperimentHandler.Environment.SERVER) {
        const key = "results";
        return await this._psychoJS.serverManager.uploadData(key, JSON.stringify(documents));
      } else offerDataForDownload("results.json", JSON.stringify(documents), "application/json");
    }
  }
  static _getLoopAttributes(loop) {
    ...
    if (typeof loop.getCurrentTrial === "function") {
      ...
      for (const trialProperty in currentTrial) attributes[trialProperty] = currentTrial[trialProperty];
    }
    return attributes;
  }
}
...
class TrialHandler extends PsychObject {
  ...
  _prepareTrialList(trialList) {
    ...
    else throw Object.assign(response, { error: "unable to prepare trial list:" + " unknown type: " + typeof trialList });
  }
  _prepareSequence() {
    ...
    if (typeof this.seed !== "undefined") Math.seedrandom(this.seed);
    ...
    if (this.method === TrialHandler.Method.SEQUENTIAL) {
      ...
      this._trialSequence = [];
      for (let i = 0; i < this.nReps; ++i) this._trialSequence.push(shuffle(indices.slice()));
    } else if (this.method === TrialHandler.Method.FULL_RANDOM) {
      let flatSequence = [];
      for (let i = 0; i < this.nReps; ++i) flatSequence.push.apply(flatSequence, indices);
      shuffle(flatSequence);
      this._trialSequence = [];
      for (let i = 0; i < this.nReps; i++) this._trialSequence.push(flatSequence.slice(i * this.nStim, (i + 1) * this.nStim));
    } else {
